...
const caseStatus = (c, r) => { const a = r.json.aggregates ?? {}; const s = c?.summary?.score; if (!c) return 'none'; if ((c.arms?.with ?? []).length && (c.arms.with.every((x) => x.isError))) return 'errored'; if (s === null || s === undefined) return 'none'; const b = base?.cases.find((x) => key(x) === key(c)); if (b && b.summary?.score !== null && s - (b.summary?.score ?? 0) < -TH) return 'regressed'; if (s < 1) return 'below'; return (c.arms?.with ?? []).some((x) => x.truncated) ? 'warn' : 'pass'; };
const runStatus = (r) => { const a = r.json.aggregates ?? {}; if (a.erroredRuns) return 'errored'; const st = r.json.cases.map((c) => caseStatus(c, r)); if (st.includes('regressed')) return 'regressed'; if (st.includes('below')) return 'below'; if (a.overallScore === null || a.overallScore === undefined) return 'none'; return st.includes('warn') ? 'warn' : 'pass'; };
...
const paths = series.map((s, si) => { const pts = s.points.filter((p) => p.score !== null); return { si, name: s.name, pts, d: pts.map((p, k) => `${k ? 'L' : 'M'}${x(p.i).toFixed(1)},${y(p.score).toFixed(1)}`).join(' '), last: pts.at(-1) }; });
...
.cells{display:flex;gap:4px;flex-wrap:wrap}.cell{width:18px;height:26px;border-radius:4px;background:var(--code);display:block;border:2px solid transparent}.cell.pass{background:var(--pass)}.cell.warn{background:var(--warn)}.cell.regressed,.cell.errored{background:var(--fail)}.cell.below{background:var(--warn);opacity:.7}.cell.none{background:var(--code)}
.cell.canary{background:transparent;border-color:var(--code)}.cell.canary.pass{border-color:var(--pass)}.cell.canary.warn,.cell.canary.below{border-color:var(--warn)}.cell.canary.regressed,.cell.canary.errored{border-color:var(--fail)}.cell:hover{outline:2px solid var(--track);outline-offset:1px}
...
${runs.length > 1 ? `<h2>Score per case over Claude Code versions</h2><div class="panel">${svg}</div>` : ''}
<h2>Runs</h2>
<div class="tablewrap"><table><thead><tr><th>status</th><th>when</th><th>track</th><th>claude code</th><th>model</th><th>overall</th>${caseNames.map((nm) => `<th>${esc(nm)}</th>`).join('')}<th>cost</th><th></th></tr></thead><tbody>${runRows}</tbody></table></div>
<p class="foot">Scores are the mean over a case's runs with the setup loaded; a drop of more than ${TH} against the pinned baseline is a regression. Filled cells are pinned-track runs, outlined cells are canaries on the alias model and latest Claude Code. Generated by <a href="https://jameskomo.github.io/config-drift-checker/">config-drift-checker</a>; every run's report lists each grader's reason.</p>
</div></body></html>`;
